        ...
        switch($this->_method){
            case "GET":
                switch($this->_complement){
                    case 0:
                        $type = ModelModel::all(0);
                        $json = $type;
                        echo json_encode($json);
                        return;
                    default:
                        $type = ModelModel::find($this->_complement);
                        if ($type==null)
                            $json = array(
            ...
            case "POST":
                $createtype = ModelModel::create($this->_data);
                $json = array(
                    "response: "=>$createtype
                );
                ...
                return;
            case "PUT":
                $createtype = ModelModel::update($this->_complement,$this->_data);
                $json = array(
                    "response: "=>$createtype
                );
                echo json_encode($json,true);
